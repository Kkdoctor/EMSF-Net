...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SIoU=False, EIoU=False, WIoU=False, Focal=False,
             alpha=1.2, gamma=0.5,  siou_dist_weight=0.6, siou_shape_weight=0.4,scale=False, ratio=1,eps=1e-7):
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or WIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU or WIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                ###已修改加入ratio###
                if Focal:
                    siou_penalty = siou_dist_weight * distance_cost + siou_shape_weight * shape_cost
                    return iou - torch.pow(siou_penalty + eps, alpha * ratio), torch.pow(inter / (union + eps),
                                                                                         gamma * ratio) # Focal_SIou
                ###已修改加入ratio###
                else:
                    siou_penalty = siou_dist_weight * distance_cost + siou_shape_weight * shape_cost
                    return iou - torch.pow(siou_penalty + eps, alpha)
            # SIou
            elif WIoU:
                if Focal:
    ...
    ###已修改加入ratio###
    if Focal:
        return iou, torch.pow(inter / (union + eps), gamma* ratio)  # Focal_IoU
    ###已修改加入ratio###
    else:
        return iou  # IoU
###修改部分###
...
class DetMetrics(SimpleClass):
    ...
    def __init__(self, save_dir=Path("../../../../../../../media/sun/5A48BE5548BE2F9D"), plot=False, on_plot=None, names={}) -> None:
